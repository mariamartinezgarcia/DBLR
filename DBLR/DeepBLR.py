...
class DeepBLR(VariationalLoss):
    ...
    def __init__(self, d, hdim_mean, hdim_var, k, input_type, percentage=0.5, cuda_device=0):
        ...
        self.optimizer_reconstruction_0 = optim.Adam(self.nn_reconstruction_0.parameters(), lr=1e-3)
        self.optimizer_reconstruction_1 = optim.Adam(self.nn_reconstruction_1.parameters(), lr=1e-3)
    ...
    def train_epoch(self, x, y, batch_size=64, mc=1, verbose=True):
        ...
        samples_weight = np.array([weight[t] for t in mask_aux_minibatch.numpy().astype('int')])
        samples_weight = torch.from_numpy(samples_weight)
        # Generate DataLoader
        sampler = torch.utils.data.WeightedRandomSampler(samples_weight, len(samples_weight))
        dataset = torch.utils.data.TensorDataset(x, y)
        dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, sampler=sampler)
        ...
        for i, (data, target) in enumerate(dataloader):
            ...
            n_imputed_samples = int(len(mask_missing_minibatch[mask_missing_minibatch == 1])*self.percentage)
            ...
            if (len(index_observed_1) - int(n_imputed_samples / 2)) < 2:
                index_imputed_0 = np.sort(np.random.choice(index_observed_0, int(n_imputed_samples), replace=False))
                mask_missing_minibatch[index_imputed_0] = 2
            elif (len(index_observed_0) - int(n_imputed_samples / 2)) < 2:
                index_imputed_1 = np.sort(np.random.choice(index_observed_1, int(n_imputed_samples), replace=False))
                mask_missing_minibatch[index_imputed_1] = 2
            else:
                index_imputed_1 = np.sort(np.random.choice(index_observed_1, int(n_imputed_samples / 2), replace=False))
                index_imputed_0 = np.sort(np.random.choice(index_observed_0, int(n_imputed_samples / 2), replace=False))
                mask_missing_minibatch[index_imputed_1] = 2
                mask_missing_minibatch[index_imputed_0] = 2
            ...
            # SGD step
            self.sgd_step(data, target_missing, mask_x=mask_x_minibatch, mask_y=mask_missing_minibatch, mc=mc)
            self.mean_ELBO_loss_epoch += (-self.ELBO_loss.cpu().data.numpy())
            self.mean_bce_y_term_epoch += self.bce_y_term.cpu().data.numpy()
            self.mean_reconstruction_x_term_epoch += self.reconstruction_x_term.cpu().data.numpy()
        ...
        self.mean_reconstruction_x_term_epoch = self.mean_reconstruction_x_term_epoch/count
