...
class VariationalLoss:
    ...
    def __init__(self, d, hdim_mean, hdim_var, k, input_type, cuda_device):
        ...
        if input_type == 'real':
            ...
            self.nn_reconstruction_0 = NeuralNetworks(k, d, hdim=hdim_mean, train_mean=True)
            self.nn_reconstruction_1 = NeuralNetworks(k, d, hdim=hdim_mean, train_mean=True)
    ...
    def sample_from_q_z(self):
        ...
        self.sample_z = self.nn_mean_z.mean + noise * torch.sqrt(torch.exp(self.nn_cov_z.logvar))
    ...
    def sample_from_y(self, N):
        ...
        output = torch.log(theta + 1e-20) - torch.log(1 - theta + 1e-20) + torch.log(noise + 1e-20) - torch.log(
            1 - noise + 1e-20)
    ...
    def bce_y(self, y, mask_y, mc=1):
        ...
        for i in range(mc):
            ...
            entropy = -self.bce_logits(z_observed @ self.sample_w,
                                       y_observed)  # minus bce to cancel the negative sign of the loss
    ...
    def reconstruction_x(self, x, y, mask_x, mask_y, mc=1):
        ...
        for i in range(mc):
            ...
            if self.input_type == 'binary':
                ...
                bce_x_0 = torch.sum(mask_x0*(-self.bce(self.nn_reconstruction_0.theta, x0)))

                self.nn_reconstruction_1.forward(z1)
                bce_x_1 = torch.sum(mask_x1*(-self.bce(self.nn_reconstruction_1.theta, x1)))

                if z_soft.shape[0] != 0:
                    # Semisupervised
                    self.nn_reconstruction_0.forward(z_soft)
                    self.nn_reconstruction_1.forward(z_soft)
                    bce_x_soft_1 = torch.sum(mask_xsoft*(-self.bce(self.nn_reconstruction_1.theta, x_soft)), 1).view(-1, 1)
                    bce_x_soft_0 = torch.sum(mask_xsoft*(-self.bce(self.nn_reconstruction_0.theta, x_soft)), 1).view(-1, 1)

                    bce_x_soft = torch.sum(y_soft*bce_x_soft_1 + (1-y_soft)*bce_x_soft_0)
            ...
            if self.input_type == 'real':
                # Reconstruction
                log_det_0 = torch.sum(mask_x0, dim=1) * torch.log(torch.tensor(2 * math.pi).to(self.device))
                log_det_1 = torch.sum(mask_x1, dim=1) * torch.log(torch.tensor(2 * math.pi).to(self.device))
                log_det_soft = torch.sum(mask_xsoft, dim=1) * torch.log(torch.tensor(2 * math.pi).to(self.device))

                self.nn_reconstruction_0.forward(z0)
                term_0 = torch.sum(log_det_0 + torch.diag((x0 - mask_x0*self.nn_reconstruction_0.mean) @ (x0 - mask_x0*self.nn_reconstruction_0.mean).T))

                self.nn_reconstruction_1.forward(z1)
                term_1 = torch.sum(log_det_1 + torch.diag((x1 - mask_x1*self.nn_reconstruction_1.mean) @ (x1 - mask_x1*self.nn_reconstruction_1.mean).T))

                if z_soft.shape[0] != 0:
                    # Semisupervised
                    self.nn_reconstruction_0.forward(z_soft)
                    self.nn_reconstruction_1.forward(z_soft)
                    term_soft_1 = torch.sum(y_soft[:, 0] * (log_det_soft + torch.diag((x_soft - mask_xsoft*self.nn_reconstruction_1.mean) @ (x_soft - mask_xsoft*self.nn_reconstruction_1.mean).T)))
                    term_soft_0 = torch.sum((1 - y_soft[:, 0]) * (log_det_soft + torch.diag((x_soft - mask_xsoft*self.nn_reconstruction_0.mean) @ (x_soft - mask_xsoft*self.nn_reconstruction_0.mean).T)))
    ...
    def kl_term_z(self):
        ...
        kl = (-1 / 2) * torch.sum(var_z + self.nn_mean_z.mean ** 2 - 1 - self.nn_cov_z.logvar)
    ...
    def kl_term_w(self):
        ...
        kl = (-1 / 2) * torch.sum(self.var_w + self.mean_w ** 2 - 1 - torch.log(self.var_w))
